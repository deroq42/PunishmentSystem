...
public class MtCommand extends Command {
    ...
    public MtCommand(String name, PunishmentSystem punishmentSystem) {
        super(name);
    ...
    public void execute(CommandSender commandSender, String[] args) {
        ...
        if (args.length != 2) {
            commandSender.sendMessage(TextComponent.fromLegacyText(Constants.PREFIX + "/mute <player> <template>"));
            return;
        }
        ...
        punishmentSystem.getPunishmentTemplateManager().getTemplate(template).thenAcceptAsync(punishmentTemplate -> {
            ...
            if(!punishmentTemplate.getPunishmentType().equals(PunishmentEntryType.MUTE.toString())) {
                commandSender.sendMessage(TextComponent.fromLegacyText(Constants.PREFIX + "Bitte gib ein valides Mute-Template an"));
                return;
            }
